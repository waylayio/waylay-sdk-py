...
def get_test_env(key: str, default: Optional[str] = None) -> str:
    ...
    if not test_var:
        raise AttributeError(
            f"{key} environment variable not configured, while test requires it."
        )
    return test_var


@pytest.fixture(scope="session", name="waylay_test_user_id")
def fixture_waylay_test_user_id():
    """Get environment variable WAYLAY_TEST_USER_ID."""
    return get_test_env("WAYLAY_TEST_USER_ID")


@pytest.fixture(scope="session", name="waylay_test_user_secret")
def fixture_waylay_test_user_secret():
    """Get environment variable WAYLAY_TEST_USER_SECRET."""
    return get_test_env("WAYLAY_TEST_USER_SECRET")


@pytest.fixture(scope="session", name="waylay_test_accounts_url")
def fixture_waylay_test_accounts_url():
    """Get environment variable WAYLAY_TEST_ACCOUNTS_URL or 'https://accounts-api-aws.dev.waylay.io'."""
    return get_test_env(
        "WAYLAY_TEST_ACCOUNTS_URL", "https://accounts-api-aws.dev.waylay.io"
    )


@pytest.fixture(scope="session", name="waylay_test_gateway_url")
def fixture_waylay_test_gateway_url():
...
@pytest.fixture(scope="session", name="waylay_test_client_credentials")
def fixture_waylay_test_client_credentials(
...
def _create_client_from_profile_or_creds(
...
def fixture_waylay_session_test_client(
    waylay_test_client_credentials: WaylayCredentials,
):
...
@pytest.fixture(name="waylay_test_client")
def fixture_waylay_test_client(waylay_test_client_credentials: WaylayCredentials):
